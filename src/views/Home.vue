...
<template>
  <div class="space-y-4">
    ...
    <div v-else-if="hasError" class="card text-center py-8">
      ...
      <p class="text-sm text-gray-500 dark:text-gray-400 mb-3">{{ t('common.loadFailed', { error: '' }) }}</p>
    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-sm font-semibold text-gray-900 dark:text-content-primary">
            {{ t('home.welcome') }}
          </h1>
          <p class="text-xs text-gray-500 dark:text-content-secondary mt-0.5">
            {{ t('home.desc') }}
          </p>
        </div>
        <button
          ...
          @click="runAutoSetup()"
          class="px-3 py-1.5 border border-primary-300 dark:border-surface-border text-primary-600 dark:text-brand-primary rounded hover:bg-primary-50 dark:hover:bg-surface-hover transition-colors text-xs flex items-center gap-1.5"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        <div class="stat-card group" @click="navigateTo('/projects')">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded bg-blue-50 dark:bg-surface-hover flex items-center justify-center shrink-0">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-lg font-bold text-gray-900 dark:text-content-primary mt-0.5">{{ stats.project_count }}</p>
            </div>
            <svg class="w-4 h-4 text-gray-300 dark:text-gray-600 group-hover:text-primary-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="stat-card group" @click="navigateTo('/plugins')">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded bg-green-50 dark:bg-green-900/20 flex items-center justify-center shrink-0">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-lg font-bold text-gray-900 dark:text-content-primary mt-0.5">{{ stats.plugin_count }}</p>
            </div>
            <svg class="w-4 h-4 text-gray-300 dark:text-gray-600 group-hover:text-primary-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="butt" stroke-linejoin="miter" stroke-width="1.5" d="M9 5l7 7-7 7" />
            </svg>
        ...
        <div class="stat-card group" @click="navigateTo('/plugins?tab=bindings')">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded bg-purple-50 dark:bg-surface-hover flex items-center justify-center shrink-0">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-lg font-bold text-gray-900 dark:text-content-primary mt-0.5">{{ stats.binding_count }}</p>
            </div>
            <svg class="w-4 h-4 text-gray-300 dark:text-gray-600 group-hover:text-primary-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="butt" stroke-linejoin="miter" stroke-width="1.5" d="M9 5l7 7-7 7" />
            </svg>
        ...
        <div class="stat-card group" @click="navigateTo('/engines')">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded bg-amber-50 dark:bg-amber-900/20 flex items-center justify-center shrink-0">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-lg font-bold text-gray-900 dark:text-content-primary mt-0.5">{{ stats.engine_count }}</p>
            </div>
            <svg class="w-4 h-4 text-gray-300 dark:text-gray-600 group-hover:text-primary-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="butt" stroke-linejoin="miter" stroke-width="1.5" d="M9 5l7 7-7 7" />
            </svg>
      ...
      <div
        ...
        class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded p-3 flex items-center gap-3 cursor-pointer hover:bg-amber-100 dark:hover:bg-amber-900/30 transition-colors"
      ...
      >
        ...
        <div class="flex-1">
          <p class="text-xs font-medium text-amber-800 dark:text-amber-300">{{ t('home.driftAlert', { count: stats.drift_count }) }}</p>
          <p class="text-[11px] text-amber-600 dark:text-amber-400 mt-0.5">{{ t('home.driftAlertDesc') }}</p>
        </div>
      ...
      <div v-if="stats.recent_projects.length > 0" class="card">
        <div class="flex items-center justify-between mb-3">
          <h2 class="text-sm font-semibold text-gray-900 dark:text-content-primary">{{ t('home.recentProjects') }}</h2>
          <div class="flex items-center gap-3">
            <div v-if="healthyCount > 0" class="flex items-center gap-1">
              <span class="w-2 h-2 rounded-full bg-status-healthy"></span>
              <span class="text-[11px] text-gray-500 dark:text-content-secondary">{{ healthyCount }}</span>
            </div>
            <div v-if="warningCount > 0" class="flex items-center gap-1">
              <span class="w-2 h-2 rounded-full bg-status-warning"></span>
              <span class="text-[11px] text-gray-500 dark:text-content-secondary">{{ warningCount }}</span>
            </div>
            <div v-if="errorCount > 0" class="flex items-center gap-1">
              <span class="w-2 h-2 rounded-full bg-status-error"></span>
              <span class="text-[11px] text-gray-500 dark:text-content-secondary">{{ errorCount }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-0">
          <div
            ...
            class="flex items-center justify-between px-2 py-1.5 rounded hover:bg-gray-50 dark:hover:bg-surface-layer transition-colors group"
          >
            <div
            ...
            >
              <div class="w-7 h-7 rounded bg-gray-50 dark:bg-surface-layer flex items-center justify-center shrink-0 overflow-hidden border border-gray-100 dark:border-surface-border">
                ...
                />
                <svg v-else class="w-4 h-4 text-gray-400 dark:text-content-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="butt" stroke-linejoin="miter" stroke-width="1.5" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                </svg>
              </div>
            ...
            <div class="flex items-center gap-2 shrink-0 ml-3">
              <button
                ...
                class="p-1.5 rounded text-gray-500 dark:text-content-muted hover:text-primary-600 dark:hover:text-brand-primary hover:bg-primary-50 dark:hover:bg-surface-hover transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
      ...
      <div v-if="!hasData" class="card">
        ...
        <div v-if="isAutoSetupRunning" class="text-center py-8">
          ...
          <p class="mt-3 text-xs text-gray-500 dark:text-gray-400">{{ autoSetupMessage }}</p>
        </div>
        <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          >
            <div class="flex items-center gap-3 mb-2">
              <div class="w-6 h-6 rounded bg-blue-50 dark:bg-surface-hover flex items-center justify-center">
                <svg class="w-3.5 h-3.5 text-blue-600 dark:text-brand-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="butt" stroke-linejoin="miter" stroke-width="1.5" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
                </svg>
              </div>
              <h3 class="text-sm font-medium text-gray-900 dark:text-content-primary">{{ t('home.step1') }}</h3>
            </div>
            <p class="text-xs text-gray-600 dark:text-content-secondary pl-10">{{ t('home.step1Desc') }}</p>
          </div>
          <div
            class="stat-card group"
          ...
          >
            <div class="flex items-center gap-3 mb-2">
              <div class="w-6 h-6 rounded bg-green-50 dark:bg-green-900/20 flex items-center justify-center">
                <svg class="w-3.5 h-3.5 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="butt" stroke-linejoin="miter" stroke-width="1.5" d="M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-1a2 2 0 10-4 0v1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 011-1h3a1 1 0 001-1V4z" />
                </svg>
              </div>
              <h3 class="text-sm font-medium text-gray-900 dark:text-content-primary">{{ t('home.step2') }}</h3>
            </div>
            <p class="text-xs text-gray-600 dark:text-content-secondary pl-10">{{ t('home.step2Desc') }}</p>
          </div>
          <div
            class="stat-card group"
            @click="navigateTo('/plugins')"
          >
            <div class="flex items-center gap-3 mb-2">
              <div class="w-6 h-6 rounded bg-purple-50 dark:bg-surface-hover flex items-center justify-center">
                <svg class="w-3.5 h-3.5 text-purple-600 dark:text-content-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="butt" stroke-linejoin="miter" stroke-width="1.5" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                </svg>
              </div>
              <h3 class="text-sm font-medium text-gray-900 dark:text-content-primary">{{ t('home.step3') }}</h3>
            </div>
            <p class="text-xs text-gray-600 dark:text-content-secondary pl-10">{{ t('home.step3Desc') }}</p>
          </div>
          <div
            class="stat-card group"
          ...
          >
            <div class="flex items-center gap-3 mb-2">
              <div class="w-6 h-6 rounded bg-amber-50 dark:bg-amber-900/20 flex items-center justify-center">
                <svg class="w-3.5 h-3.5 text-amber-600 dark:text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="butt" stroke-linejoin="miter" stroke-width="1.5" d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
              </div>
              <h3 class="text-sm font-medium text-gray-900 dark:text-content-primary">{{ t('home.step4') }}</h3>
            </div>
            <p class="text-xs text-gray-600 dark:text-content-secondary pl-10">{{ t('home.step4Desc') }}</p>
          </div>
        </div>
        <div class="mt-4 flex flex-wrap gap-2 justify-center">
          <div class="flex items-center gap-1.5 text-[11px] text-gray-400 dark:text-content-muted">
            <kbd class="px-1.5 py-0.5 rounded bg-gray-100 dark:bg-surface-hover border border-gray-200 dark:border-surface-border font-mono text-[10px]">Ctrl+K</kbd>
