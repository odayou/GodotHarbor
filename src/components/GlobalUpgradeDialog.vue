  ...
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" @click="close">
      <div class="dialog-container w-full max-w-md" @click.stop>
        ...
        <div class="mb-4">
          <div class="flex items-center gap-2 mb-3">
            <svg class="w-5 h-5 text-primary-600 dark:text-brand-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="butt" stroke-linejoin="miter" stroke-width="1.5" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M9 19l3 3m0 0l3-3m-3 3V10" />
            </svg>
            <span class="text-base font-medium text-gray-900 dark:text-content-primary">{{ pluginName }}</span>
          </div>

          <div class="p-3 bg-gray-50 dark:bg-surface-hover rounded mb-3">
        ...
        <!-- Results -->
        <div v-if="hasUpgraded && upgradeResults.length > 0" class="mb-4 p-3 rounded border"
        ...
        <div class="flex gap-3">
          <button @click="close" class="flex-1 py-2.5 text-sm font-medium rounded border border-gray-300 dark:border-surface-border text-gray-700 dark:text-content-primary hover:bg-gray-50 dark:hover:bg-surface-layer transition-colors">
