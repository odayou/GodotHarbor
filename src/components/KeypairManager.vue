...
<template>
  <Teleport to="body">
      ...
      <div class="relative bg-white dark:bg-surface-card rounded-2xl shadow-2xl max-w-lg w-full mx-4 max-h-[80vh] flex flex-col">
        <div class="p-6 pb-4 border-b border-gray-200 dark:border-surface-border">
          <div class="flex items-center justify-between">
            <h2 class="text-lg font-bold text-gray-900 dark:text-content-primary">签名密钥管理</h2>
          ...
          <p class="text-sm text-gray-500 dark:text-content-muted mt-1">
            管理用于签名模板的 Ed25519 密钥对。签名可验证模板来源和完整性。
          </p>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div class="p-4 rounded-lg bg-gray-50 dark:bg-surface-layer">
            <h3 class="text-sm font-semibold text-gray-900 dark:text-content-primary mb-3">生成新密钥对</h3>
            <div class="flex gap-2">
              <input
                v-model="newKeypairName"
                type="text"
                placeholder="签名者名称（如：你的名字或组织名）"
          ...
          <div v-else-if="keypairs.length === 0" class="text-center py-8">
            ...
            <p class="text-sm text-gray-500 dark:text-content-muted">暂无密钥对，请先生成</p>
          </div>
          ...
          <div v-else class="space-y-2">
            ...
            >
              ...
              <div v-if="expandedPublicKey === kp.public_key" class="mt-2">
                <p class="text-xs text-gray-500 dark:text-content-muted mb-1">公钥:</p>
    ...
    <ConfirmDialog
      ...
      title="删除密钥对"
      description="删除后无法恢复，使用此密钥签名的模板将无法验证。确定要删除吗？"
      confirm-text="删除"
      confirm-color="red"
